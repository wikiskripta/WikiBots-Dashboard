...
class TaskLog extends Controller
{
    ...
    public function process(array $args = []): int
    {
        ...
        $allowedGroups = $pm->getAllowedLogGroups(IniType::TASK_LOG, $task->getUrl());
        ...
        if (!$um->isUserLoggedIn()){
            ...
            if (empty($args)) {
                self::$data['layout']['title'] = 'Záznamy spuštění úkonu '.$task->getUrl();
                self::$views[] = 'logs';

                $logDir = 'Tasks'.DIRECTORY_SEPARATOR.$task->getUrl().DIRECTORY_SEPARATOR;

                $lm = new LogManager();
                self::$data['logs']['processname'] = $task->getUrl();
                ...
                $log = 'Tasks'.DIRECTORY_SEPARATOR.$task->getUrl().DIRECTORY_SEPARATOR.$logfile;
